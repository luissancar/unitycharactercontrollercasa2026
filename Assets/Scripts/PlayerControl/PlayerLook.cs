...
public class PlayerLook : MonoBehaviour // Declara la clase PlayerLook, que hereda de MonoBehaviour.
{
    ...
    private void Awake() // Awake se llama al inicializar el objeto, antes de Start.
    {
        ...
        // Si no lo asignas a mano, intenta coger el PlayerInput del mismo objeto
        if (playerInput == null)
            playerInput = GetComponent<PlayerInput>();

        // Nada más cargar la escena, desactivamos el input
        if (playerInput != null)
        {
    ...
    IEnumerator StartInput() // lo ejecuta automáticamente como corrutina al iniciar la escena.
    {
        // Esperamos X segundos antes de permitir input
    ...
    private void
        OnLook(InputValue value) // Método llamado automáticamente por PlayerInput cuando se activa la acción "Look".
    {
    ...
    private void HandleLook() // Método que implementa la lógica de mirar con el ratón/stick.
    {
        float mouseX =
            ...
            Time.deltaTime; // Calcula la rotación horizontal según el input y la sensibilidad.
        float mouseY =
            lookInput.y * mouseSensitivity *
            Time.deltaTime; // Calcula la rotación vertical según el input y la sensibilidad.
