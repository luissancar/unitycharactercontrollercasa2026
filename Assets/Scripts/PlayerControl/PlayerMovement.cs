...
public class PlayerMovement : MonoBehaviour     // Declara la clase PlayerMovement, que hereda de MonoBehaviour.
{
    ...
    {
        characterController = GetComponent<CharacterController>(); // Obtiene el CharacterController adjunto al mismo GameObject.
    }
    ...
    private void HandleMovement()               // Método que implementa la lógica de movimiento, salto y gravedad.
    {
        ...
        // Movimiento local XZ
        Vector3 localMove = new Vector3(moveInput.x, 0f, moveInput.y); // Crea un vector de movimiento en el espacio local (X y Z, sin Y).

        // Convertir de local a mundo según la orientación del player (yaw la controla PlayerLook)
        Vector3 worldMove = transform.TransformDirection(localMove); // Convierte el vector local a espacio global usando la rotación del jugador.
        ...
        Vector3 horizontalVelocity = worldMove * moveSpeed; // Multiplica la dirección por la velocidad para obtener la velocidad horizontal.
